...
表の上には**フロア帯**が乗ります。側面から見たドット絵の部屋で、1 プロジェクト 1 台の机が並びます。帯が伝えるのは
6 つだけです。生きているか／どこに居るか／呼んでいるか／どのベンダーか／何が変わったか（❗が出たロボは受付まで
歩いていきます）／留守のあいだに何が片付いたか。**帯には文字を 1 つも描きません**。名前も数字も表の仕事です。
行にカーソルを乗せると、その 1 体の足元が光ります。部屋は窓の幅いっぱいに伸び、スマホでは右端が切れた部屋を
出すかわりに帯ごと畳みます。絵は画像ファイルではありません。1 コマずつがソースの中の文字グリッドなので、
コードと同じように差分が読めます。
