...
- **上質な3Dの、置いておける画面。** 白×薄ラベンダーのガラスロフトに寒色の昼光、陶器のロボ、青白く光る黒いモニタ（発光画面の実ブルーム・ビネット・静止グレイン・PMREM の小部屋環境マップ）。同時稼働の最大値で S/M/L/XL（24席）に広がり、レベルで装飾が解錠されます。配信にもセカンドモニタにも使えます。
