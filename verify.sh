...
#   modules_data.js raw ≤ 3.5MB / gzip ≤ 1.2MB（R96-D で 2.5MB/900KB から拡張＝Tripo 生成什器の PWA 同梱分。
#   PWA に載せる物は ui/iso/gen/manifest.json の pwa:true だけ・残りは null スタブ）。
#   ui/iso*/tex は合計 ≤ 600KB・1枚 ≤ 80KB（GPT-Image デカールを足すときの天井）。
#   ui/iso/gen/*.js（生成什器）は 1 点 ≤ 160KB・合計 ≤ 3.5MB（デスクトップはローカル配信＝転送は初回だけ）。
python3 - <<'EOF' || NG=$((NG+1))
...
if m.is_file():
    ...
    if len(raw) > 3_500_000: bad.append(f"modules_data.js raw {len(raw)//1024}KB > 3500KB")
    if gz > 1_200_000: bad.append(f"modules_data.js gzip {gz//1024}KB > 1200KB")
...
gen = [p for p in root.glob("ui/iso/gen/*.js") if p.name != "index.js"]
gtotal = sum(p.stat().st_size for p in gen)
for p in gen:
    if p.stat().st_size > 160_000: bad.append(f"{p} {p.stat().st_size//1024}KB > 160KB（三角形を減らす）")
    if not p.read_text(encoding="utf-8").startswith("// generated by tools/glb_to_geom.py"):
        bad.append(f"{p} は tools/glb_to_geom.py の生成物ではない")
if gtotal > 3_500_000: bad.append(f"ui/iso/gen 合計 {gtotal//1024}KB > 3500KB")
print(f"  ✓ ui/iso/gen 生成什器 {len(gen)}点 合計 {gtotal//1024}KB")
