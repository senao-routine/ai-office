...
R=$(curl -s -X POST "$B/status" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"office":{"employees":[{"session":"e2e-attn-0001","disp":"E2E検証部","state":"working","verb":"検証中","question":"どの案で進めますか?","questionOptions":[{"label":"案A (Recommended)","desc":"推奨案"},{"label":"案B","desc":"別案"},{"label":"案C","desc":"保留案"}]},{"session":"oc-e2e","disp":"OpenClaw","dept":"OpenClaw","state":"working","verb":"replying","external":"openclaw"}]}}')
