...
R=$(curl -s -X POST "$B/status" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"office":{"officeName":"E2Eオフィス","tasks":{"pending":2,"inProgress":1,"completed":3},"res":{"fiveHour":21,"sevenDay":58,"staleSec":30,"providers":[{"id":"claude","label":"Claude","bars":[{"k":"5h","pct":21},{"k":"7d","pct":58}],"staleSec":30},{"id":"codex","label":"Codex","bars":[{"k":"7d","pct":7}]}]},"templates":[{"label":"ビルド確認","text":"verifyを回して結果を1行で報告して"}],"employees":[{"session":"e2e-attn-0001","disp":"E2E検証部","state":"working","verb":"検証中","question":"どの案で進めますか?","questionOptions":[{"label":"案A (Recommended)","desc":"推奨案"},{"label":"案B","desc":"別案"},{"label":"案C","desc":"保留案"}]},{"session":"oc-e2e","disp":"OpenClaw","dept":"OpenClaw","state":"working","verb":"replying","external":"openclaw"}]}}')
