...
if [ -f "$PLIST" ]; then
  ...
  # R97-A: ここが info（=ng を増やさない）だったので、配達員が居ないまま「✅ 中継の準備ができました」に
  # 到達し、案内どおりペアリングしたのに**スマホからの指示が永久に届かない**状態に入れた。
  bad "配達エージェントの常駐がありません → 先に bash setup.sh を実行してください（これが無いとスマホからの指示は届きません）"
fi
...
if [ "$ng" -eq 0 ]; then
  ...
  say ""
  say "  ※ スマホ連携を含む全機能が無料で使えます（ライセンス不要）。"
  exit 0
fi
